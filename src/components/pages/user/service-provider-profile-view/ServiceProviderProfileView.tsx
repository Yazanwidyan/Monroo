  ...
  <Box mt={2} mb={4}>
    ...
    <Box display="flex" alignItems="center">
      {instagram && (
        <Link
          href={`${
            !instagram.startsWith("http") ? "" : "https://www.instagram.com/"
          }${instagram}`}
          target="_blank"
          mx={2}
        >
      ...
      {linkedin && (
        <Link
          href={`${
            !linkedin.startsWith("http") ? "" : "https://www.linkedin.com/in/"
          }${linkedin}`}
          target="_blank"
          mx={2}
        >
...
const AdditionalInfo = ({
  ...
  // Check if any of the props exist
  ...
  if (!showAdditionalInfo) {
    return null; // Don't render anything if none of the props exist
  }
    ...
    <Box mb={4}>
      ...
      <Grid templateColumns="repeat(2, 1fr)" gap={2}>
        <Box>
          ...
          {musicGenres?.length && musicGenres?.[0] !== null ? (
            <MusicGenreLookup value={musicGenres} />
          ) : null}
          {musicalInstruments?.length && musicalInstruments?.[0] !== null ? (
...
const ServiceProviderProfileView = () => {
  ...
  const fetchReviews = async () => {
    ...
    try {
      ...
      setReviews(res.data);
      const totalStars = res?.data?.reduce(
      ...
      <Container maxW={"6xl"}>
        <Grid templateColumns="1fr 3fr" gap={8}>
          <GridItem colSpan={1}>
            <Box>
              <Image
                ...
                onClick={() =>
                  handleImageClick(
                    profilePic || "/assets/images/userprofile.jpg"
                  )
                }
                height={300}
                width={"100%"}
          ...
          <GridItem colSpan={1}>
            ...
            {instagram || linkedin || youtubelink ? (
              <SocialMediaLinks
                instagram={instagram}
                linkedin={linkedin}

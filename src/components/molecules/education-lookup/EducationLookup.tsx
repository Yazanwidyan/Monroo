        ...
        if (Array.isArray(val)) {
            return val
                .map((v) => {
                    ...
                    return education ? (i18n?.language?.includes('en') ? education.name : i18n?.language?.includes('ar') ? education.nameAR : education.nameRUS) : v;
                })
                .join(', ');
        } else {
            const education = educationData.find((item) => item.code === val);
            return education ? (i18n.language == 'en' ? education.name : i18n.language == 'ar' ? education.nameAR : education.nameRUS) : val;
        }
    };
